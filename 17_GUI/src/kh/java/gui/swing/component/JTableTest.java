...
public class JTableTest extends JFrame {
	...
	public JTableTest(int w, int h, String title) {
		...
		// 컬럼정보
		Object[] columnNames = { "이름", "주소", "나이", "결혼여부" };
		...
		//viewport지정
		table.setPreferredScrollableViewportSize(new Dimension(500,100)); //너비객체를 추가하면서 
...
//				System.out.println(row + " ," + col);
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int rowLen = model.getRowCount();// 길이를 구하는 키워드?
...
//						System.out.println(data);
						switch(j) {
						case 0: newMember.setName((String)data);break; //문자열로
						case 1: newMember.setAddr((String)data);break; //
						case 2: newMember.setAge(Integer.parseInt(data.toString()));break; //문자열이었다가 정수형으로
						case 3: newMember.setMaried((boolean)data);break; //마지막엔 불린으로 형변환
						}
					}
